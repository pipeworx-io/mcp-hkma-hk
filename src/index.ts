...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Hong Kong Monetary Authority');
}
...
const tools: McpToolExport['tools'] = [
  ...
  {
    name: 'hkma_interbank_interest_rates',
    description:
...
async function hkmaGet(path: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  const res = await pwFetch(`${BASE}/${path}${qs ? `?${qs}` : ''}`, {
